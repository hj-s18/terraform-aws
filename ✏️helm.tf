# EKS가 완전히 생성 후 Helm provider가 실행되도록 하기 위해 생성한 data source
...
provider "kubernetes" {
  host                   = data.aws_eks_cluster.tf_eks_cluster.endpoint   # privider에는 depends_on 사용 못하므로 data source 활용
